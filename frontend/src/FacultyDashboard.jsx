...
const FacultyDashboard = () => {
  ...
  const handleSearch = async () => {
    ...
    if (data && data.top_matches) {
        ...
        setAllData({
            // Δημιουργούμε τη δομή που περιμένουν τα Charts/Tables components σου
            "Classification_Results": { 
                hyperparameters_results: data.hyperparameters,  
                matching_results: data.top_matches,        // Διόρθωση: top_matches αντί για matches
                recommended_results: data.recommended_matches,
                execution_time: data.execution_time,
                
                // ΠΡΟΣΟΧΗ: Εδώ πρέπει να βάλεις το όνομα του κλειδιού JSON, όχι της Python μεταβλητής
                lsh_brp: data.lsh_brp,                     // Διόρθωση: data.lsh_brp (όχι _dict)
                lsh_minihash: data.lsh_minihash,           // Διόρθωση: data.lsh_minihash (όχι _dict)
                
                scatter_plot: data.visual_data
            }
        });
      }
